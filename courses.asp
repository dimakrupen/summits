...
function EditFile(catid, id)
{
	...
	else
	{
		...
		document.all["ifFileAdd" + catid].height = 20;
	}
}
...
<img src="images/cap_Courses.gif"><input type="button" name="btnSubmit1" value="Add Category" onclick="fnAdd();" class="tblBase">
...
<%
Dim vipCode
strSQL = "SELECT vipcode FROM Settings"
%><!--#include file="includes/rsopen.asp"--><%
vipCode = rs("vipcode")
rs.Close
%>
You may take these courses at no charge (in the checkout screen use VIP
Code: <%=vipCode%> to void any charges).</small><br><br>
<%
strSQL = "SELECT * FROM Categories WHERE CategoryTypeID = 2"
%><!--#include file="includes/rsopen.asp"-->
		...
		%>
			<tr>
				<td>&nbsp;&nbsp;<img src="images/arrow.gif">&nbsp;
					<a target="_blank" href="<%=rs2("filename")%>"><%=rs2("description")%></a>
				</td>
