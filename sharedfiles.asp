...
<img src="images/cap_sharedfiles.gif"><input type="button" name="btnSubmit1" value="Add Category" onclick="fnAdd();" class="tblBase">
<iframe src="_tip.htm" id="ifHidden" name="ifHidden" marginwidth="0" marginheight="0" frameborder="0" scrolling="no" width="500px" height="40px"></iframe>
<%
strSQL = "SELECT * FROM Categories WHERE CategoryTypeID = 0"
...
Do While Not rs.EOF
	%>
		<tr>
			<td><b><%=rs("Category")%></b>&nbsp;&nbsp;<a href="javascript:fnRename(<%=rs("id")%>);">Rename</a>&nbsp;&nbsp;<a href="javascript:DeleteCategory(<%=rs("id")%>);">Delete</a></td>
		...
		%>
			<tr>
				<td>&nbsp;&nbsp;<img src="images/arrow.gif">&nbsp;<%=rs2("description")%><br>
					&nbsp;&nbsp;&nbsp;&nbsp;<a target="_blank" href="files/<%=rs2("filename")%>"><%=rs2("filename")%></a>
				</td>
...
If sesClass2 = "Admin" Then
	strSQL = "SELECT SharedStuff.*, userdata.first + ' ' + userdata.last as userdeleted, Categories.Category " & _
		...
		" WHERE SharedStuff.deleted = True " & _
		" AND (mygrouponly = False OR postedby IN " &_
